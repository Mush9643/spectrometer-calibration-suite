...
def highlight_rn_peaks(chart, series, peak_points, parent_window=None, use_three_peaks=True):
    """Отмечает пики Rn, вычисляет линейную регрессию, сохраняет коэффициенты и вычисляет ra, am_rate и k1p9."""
    ...
    if third_peak and use_three_peaks:  # Добавляем третий пик только если use_three_peaks=True
    ...
    if third_peak and use_three_peaks:
    ...
    if len(x_values) > 0:
        ...
        for e in P90:
            ...
            if e == 2700 and parent_window is not None:
                parent_window.beta_p2700 = p_value
...
def calculate_ra(parent_window):
    ...
    if len(ch_values) < 4:
        print("Ошибка: Недостаточно значений каналов для вычисления ra.")
        return

    ch0, ch2, ch3 = ch_values[0], ch_values[2], ch_values[3]
    ...
    for series_name, series in parent_window.alfa_series_dict.items():
        if "Rn" in series_name:
            ...
            points = series.points()
            rn_numerator_values = [p.y() for p in points if ch0 <= p.x() < ch2]
            rn_denominator_values = [p.y() for p in points if ch2 <= p.x() < ch3]

            if rn_numerator_values:
                numerator_sum += np.sum(rn_numerator_values)
                print(f"Сумма Rn_i для числителя серии {series_name}: {np.sum(rn_numerator_values)}")
            if rn_denominator_values:
                denominator_sum += np.sum(rn_denominator_values)
                print(f"Сумма Rn_i для знаменателя серии {series_name}: {np.sum(rn_denominator_values)}")

    if found_rn_series and denominator_sum != 0:
        ra = numerator_sum / denominator_sum
        print(f"\nЗначение ra: {ra:.3f}")
        parent_window.ra_value = ra
    else:
        print("Ошибка: Не удалось вычислить ra.")
...
def calculate_am_rate(parent_window):
    ...
    if not hasattr(parent_window, 'alfa_data_arrays') or not parent_window.alfa_data_arrays:
        print("Ошибка: Нет данных в alfa_data_arrays.")
        return

    if not hasattr(parent_window, 'alfa_series_dict') or not parent_window.alfa_series_dict:
        print("Ошибка: Нет данных в alfa_series_dict.")
        return
    ...
    if len(ch_values) < 3:
        print("Ошибка: Недостаточно значений каналов.")
        return

    ch0, ch2 = ch_values[0], ch_values[2]
    ...
    for series_name, series in parent_window.alfa_series_dict.items():
        if "Am241" in series_name:
            ...
            points = series.points()
            numerator_sum += sum(p.y() for p in points if ch0 <= p.x() <= ch2_ceil)
            print(f"Сумма M2_i для серии {series_name}: {numerator_sum}")

    if not found_am241_series:
        print("Предупреждение: Не найдено серий с 'Am241'.")
        return
    ...
    for file_name, data in parent_window.alfa_data_arrays.items():
        if "Am241" in file_name:
            ...
            print(f"M_0 из файла {file_name}: {denominator_m0}")
            break

    if denominator_m0 == 0:
        print("Ошибка: Знаменатель M_0 равен 0.")
        return
...
def calculate_k1p9(parent_window):
    ...
    if not hasattr(parent_window, 'am_rate_value'):
        print("Ошибка: Значение am_rate отсутствует.")
        return

    am_rate = parent_window.am_rate_value
    if am_rate == 0:
        print("Ошибка: am_rate равен 0.")
        return
...
def add_calibration_button(window):
    ...
    # Создаём отдельный layout для кнопки "Калибровка"
    ...
    # Проверяем, есть ли уже controls_layout (где находятся другие элементы управления)
    controls_layout = None
    for i in range(main_layout.count()):
        item = main_layout.itemAt(i)
        if isinstance(item.layout(), QHBoxLayout) and item.layout().itemAt(0).widget().text() in ["Логарифмический масштаб"]:
            controls_layout = item.layout()
            break

    if not controls_layout:
        # Если controls_layout не найден, создаём новый для других элементов управления
        controls_layout = QHBoxLayout()
        controls_layout.addWidget(window.log_checkbox if hasattr(window, 'log_checkbox') else QWidget())
        controls_layout.addStretch()
        main_layout.addLayout(controls_layout)

    # Добавляем layout с кнопкой "Калибровка" в самый низ основного layout
...
def open_calibration_dialog(window):
    ...
    window.calibration_performed = True
    dialog = CalibrationDialog(window)
    dialog.exec()

def recalculate_with_two_peaks(window):
    ...
    chart = window.chart
    for series_name, peaks in window.peak_points.items():
        if "Rn" in series_name:
            for peak_name, peak_series in peaks.items():
                chart.removeSeries(peak_series)
            # Очищаем словарь пиков для этой серии
            window.peak_points[series_name] = {}

    # Перерисовываем пики и пересчитываем регрессию
    for series_name, series in window.alfa_series_dict.items():
        if "Rn" in series_name:
            highlight_rn_peaks(chart, series, window.peak_points, window, use_three_peaks=window.use_three_peaks)

    # Обновляем линии P90
    window.highlight_p90_points()

    # Обновляем таблицу калибровки
...
def add_recalculate_button(window):
    ...
    # Устанавливаем стиль кнопки: серый фон (#4A4A4A) и белый текст для читаемости
    button.setStyleSheet("background-color: #4A4A4A; color: #FFFFFF; border: none; border-radius: 5px; padding: 8px 16px; font-size: 12px; font-weight: 600;")
